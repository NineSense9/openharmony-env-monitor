...
export const SpatialAttitudeCard: React.FC<SpatialAttitudeCardProps> = ({ telemetry, systemState }) => {
  ...
  const bubbleX = Math.max(-18, Math.min(18, ax * 18));
  const bubbleY = Math.max(-18, Math.min(18, -ay * 18));
  ...
  return (
    <div className="glass-panel rounded-xl p-3 flex flex-col justify-between bg-[#060D1A]/90 border border-cyan-500/30 flex-1 min-h-0 overflow-hidden shadow-[0_0_20px_rgba(0,240,255,0.06)]">
      ...
      {/* 2. Middle Row: Horizon + Bubble Level (Spacious & Centered) */}
        ...
        <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-2 flex flex-col items-center justify-between relative overflow-hidden">
          <div className="w-full flex items-center justify-between text-[10px] font-mono text-slate-400">
            <span className="flex items-center gap-1 text-cyan-300">
              <Crosshair className="w-3 h-3" /> 姿态盘
            </span>
            <span className="text-slate-500">PITCH/ROLL</span>
          </div>

          <div className="w-[92px] h-[92px] rounded-full border-2 border-cyan-500/40 relative overflow-hidden bg-slate-900 shadow-inner my-1">
            <div
              className="absolute w-[160px] h-[160px] -top-[34px] -left-[34px] transition-transform duration-150 ease-out"
              style={{
                transform: `rotate(${-rollClamped}deg) translateY(${pitchClamped * 0.8}px)`
              }}
            >
              <div className="w-full h-[80px] bg-gradient-to-t from-sky-700/80 to-sky-950/90 border-b border-cyan-300/80" />
              <div className="w-full h-[80px] bg-gradient-to-b from-amber-950/80 to-slate-950 border-t border-amber-500/50" />
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-[7px] font-mono text-cyan-200/70">
                <div className="w-6 border-b border-cyan-200/50 mb-2" />
                <div className="w-10 border-b border-cyan-100 font-bold" />
                <div className="w-6 border-b border-cyan-200/50 mt-2" />
              </div>
            </div>
        ...
        <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-2 flex flex-col items-center justify-between">
          <div className="w-full flex items-center justify-between text-[10px] font-mono text-slate-400">
            <span className="flex items-center gap-1 text-emerald-400">
              <Activity className="w-3 h-3" /> 水准仪
            </span>
            <span className="text-slate-500">|G|={totalG.toFixed(2)}g</span>
          </div>

          <div className="w-[82px] h-[82px] rounded-full border border-slate-700 bg-slate-900/90 relative flex items-center justify-center shadow-inner my-1">
          ...
          <div className="w-full flex items-center justify-between text-[9px] font-mono text-slate-400 px-1">
            <span className="text-cyan-400">AX:{ax.toFixed(2)}</span>
            <span className="text-blue-400">AY:{ay.toFixed(2)}</span>
            <span className="text-purple-400">AZ:{az.toFixed(2)}</span>
          </div>
        </div>
      </div>

      {/* 3. Bottom Row: I2C0 Bus Matrix */}
      <div className="bg-slate-950/50 border border-slate-800/80 rounded-lg p-1.5 flex flex-col gap-1 shrink-0">
