          ...
          <div className="flex-1 grid grid-cols-12 gap-3 mt-2">
            ...
            <div className="col-span-5 bg-[#000814] border-2 border-cyan-400/60 rounded-xl p-2.5 flex flex-col justify-between shadow-[0_0_20px_rgba(0,240,255,0.2)]">
              ...
              <div className="grid grid-cols-2 gap-1.5 py-1 text-[9px] font-mono">
                ...
                {/* 象限 4：按键 */}
                <div className="bg-[#071326] border border-cyan-500/20 p-1.5 rounded">
                  <span className="text-cyan-400 block text-[8px]">KEY & BUS</span>
                  <span className="text-amber-300 block mt-0.5 font-bold">
                    LAST: {lastKey}
                  </span>
                  <div className="text-[8px] text-slate-400">
                    ADC5: 0~3.3V
              ...
              {/* LCD 底栏按键提示 */}
              <div className="bg-[#001026] text-[8px] font-mono text-cyan-300/80 px-2 py-0.5 rounded flex justify-around border-t border-cyan-900/50">
                <span>[K3:消警]</span>
                <span>[K4:调速]</span>
                <span>[K5:自检]</span>
                <span>[K6:扫描]</span>
            ...
            <div className="col-span-3 flex flex-col justify-between space-y-2">
              ...
              {/* SARADC5 K3 ~ K6 四实体微动轻触按键矩阵 */}
              <div className="bg-[#050B14] border border-cyan-500/30 rounded-lg p-2 flex flex-col justify-between">
                <div className="flex justify-between items-center text-[9px] font-mono text-slate-300 mb-1">
                  <span>ADC KEYPAD</span>
                  <span className="text-[8px] text-amber-400">GPIO0_PC5</span>
                </div>
                
                <div className="grid grid-cols-2 gap-1.5">
                  {(['K3', 'K4', 'K5', 'K6'] as const).map((k) => {
                    const isPressed = activeKey === k || telemetry?.last_key === k;
                    ...
                    return (
                      <button
                        ...
                        className={`py-1 px-1 rounded text-center border font-mono transition-all active:scale-95 ${
                          isPressed
                            ? 'bg-amber-400 text-slate-950 border-amber-300 shadow-[0_0_8px_#F59E0B] font-bold'
                            : 'bg-slate-900 text-cyan-300 border-cyan-800/60 hover:border-cyan-400'
                        }`}
                      >
                        <div className="text-[10px] font-bold">{k}</div>
                        <div className="text-[7px] text-slate-400">{hints[k]}</div>
                      </button>
                    );
                  })}
                </div>
